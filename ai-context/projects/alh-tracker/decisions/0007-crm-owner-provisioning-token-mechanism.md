...
3. Tracker backend executes all of the following:
   a. Creates or resolves the tracker `Facility` record. **RESOLVED by ADR 0009 (2026-05-19):** The Facility record is created by this provisioning call in `pending_setup` state, keyed by `crm_facility_reference = X-CRM-Facility-Id`. If a Facility with that reference already exists (retry scenario), it is reused without creating a duplicate. See ADR 0009 for the full creation sequence, allowed fields, Facility status lifecycle, and idempotency specification.
...
- **RESOLVED — Facility record creation at provisioning (ADR 0009, 2026-05-19):** The tracker `Facility` record is created by the provisioning API call — not pre-existing. The endpoint atomically creates or reuses a `Facility` in `pending_setup` state keyed by an opaque `crm_facility_reference` (derived from `X-CRM-Facility-Id`). Allowed CRM-to-tracker facility fields: `facility_name`, `facility_city`, `facility_state`, `license_number` (optional). `Facility.capacity` (licensed capacity), subscription resident limit, and allocated resident count are not set at provisioning time. `Facility.provisioning_status` transitions from `pending_setup` to `active` on owner activation. See ADR 0009 for full specification.
