...
- **Mechanism (ADR 0007 — accepted):** Custom `provisioning_tokens` table selected. CRM calls the tracker provisioning API endpoint; tracker backend manages all token and account lifecycle logic. CRM receives only an opaque `provisioning_reference` and `provisioning_status`. See ADR 0007.
- **Facility creation (ADR 0009 — accepted):** The provisioning API call creates the tracker `Facility` record in `pending_setup` state atomically with the `User` row and `ProvisioningToken`. CRM forwards facility identification fields only: `facility_name`, `facility_city`, `facility_state`, `license_number` (optional). `Facility.capacity` (licensed capacity), subscription resident limit, and allocated resident count are not set at provisioning time. Owner completes facility setup post-activation. See ADR 0009.
- **Authentication (ADR 0008 — accepted):** Rotating static API key (MVP). CRM stores raw key in Vercel env vars; tracker stores SHA-256 hash only. See ADR 0008.
...
- TODO: Transactional email service selection is unresolved. See ADR 0007 Open Implementation TODOs.
