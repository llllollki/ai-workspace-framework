...
**Internal CRM:** An internal ALH Tracker business tool for managing commercial relationships — facility owner/customer profiles, facility records, allowable resident count configuration, onboarding tracking, payment/subscription status, communications with facility owners, and support/admin notes. The CRM also provisions the facility owner's Facility Tracker App account as part of onboarding: CRM staff create an invited account in the tracker app and the system sends an activation deep link to the owner's email. This provisioning is a forward write only — CRM staff gain no access to tracker app care data, resident records, or shift logs. The CRM is not a care-delivery surface. CRM users are ALH Tracker business/admin staff; they are a distinct principal class separate from facility owners, caregivers, and family members. The CRM does not expose resident wellness/care logs. See ADR 0005 and ADR 0006.
...
**Family Member App:** Planned Phase 2 separate mobile/tablet surface. Family members may create a `FamilyUser` account (identity only) before the facility owner/admin has approved their access — account creation does not grant any resident data access. A family member with an active `FamilyAccessConsent` record (granted by the facility owner/admin per ADR 0004) gains read-only access to an approved wellbeing view for the specific resident(s) they are authorized for. Family app data access architecture is governed by ADR 0004 and ADR 0006. App delivery format (PWA vs. native) is a pending ADR; authentication model and onboarding invite mechanism are TODO pending design and counsel review.
