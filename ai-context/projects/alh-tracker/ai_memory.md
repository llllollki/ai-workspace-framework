...
- **CRM-to-tracker API authentication:** The tracker provisioning API endpoint must authenticate requests from the CRM. Options: rotating API key (simplest) or short-lived service JWT (more secure). Unresolved — must be decided before the provisioning API is built. See ADR 0007.
