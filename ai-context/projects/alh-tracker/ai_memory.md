...
- **CRM-to-tracker provisioning handshake (conceptual flow resolved in ADR 0006 — proposed; implementation mechanism unresolved):** ADR 0006 documents the provisioning flow at the conceptual level: CRM staff creates an `invited` account in the tracker app → the system sends a confirmation email → owner activates via opaque, expiring, one-time-use deep link. The open question is the specific implementation mechanism: (a) Supabase Auth invite API triggered server-side; (b) a custom `provisioning_tokens` table populated by a CRM-to-tracker service; (c) a manual step using a tracker app admin interface separate from the CRM. This choice has security and architecture implications and must be resolved before Phase 2 implementation.
- **iOS Universal Links vs. Android App Links:** The owner activation deep link behaves differently on iOS (Universal Links — requires Apple App Site Association file on the server) and Android (App Links — requires Digital Asset Links file). These mechanisms have different trust models and different server-side configuration requirements. This must be resolved before the app is submitted to the stores.
- **Supabase Auth invite API compatibility:** If the Supabase Auth `inviteUserByEmail` approach is chosen for provisioning, the CRM must trigger this API call server-side (not from a browser client). Whether a CRM-to-tracker service boundary or a shared server-side function is the right architecture for this call is unresolved.
