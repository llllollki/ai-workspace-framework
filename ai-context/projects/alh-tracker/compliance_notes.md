...
| CRM-to-tracker provisioning API authentication | Required — the tracker provisioning endpoint must authenticate requests from the CRM using a dedicated credential scoped only to provisioning actions. The credential must be stored server-side only (never in browser code, never committed to source control). The CRM must never receive the tracker Supabase service-role key. The endpoint must not return resident care data, family data, or any care-operations entity. MVP: rotating static API key. Phase 2 hardening: HMAC-signed short-lived service JWT. See ADR 0008. |
| Provisioning-state RLS (pending_setup) | Required — all care-ops tables block any client access when `Facility.provisioning_status != 'active'` or `User.account_status != 'active'`. `invited` and `password_pending` users have no Supabase session (ADR 0007); RLS is a defensive layer. `ProvisioningToken` and `ProvisioningEvent` have zero client-accessible policies. Implemented via `is_active_user_on_active_facility()` helper in all care-ops table policies. See ADR 0010 (accepted). |
